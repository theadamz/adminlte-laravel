...
use App\Http\Requests\Basic\BrandRequest;
...
{
    protected string $templateName = "template_brands.xlsx";

    public function index()
    {
        // js
        GeneralHelper::addAdditionalJS([
            'resources/js/pages/basic/brand.js'
        ]);

        // template name
        $templateName = $this->templateName;

        return view('basic.brand')->with(compact('templateName'));
    }

    public function datatable(Request $request): JsonResponse
    {
        $queries = Brand::selectRaw("id, code, name, is_active")
    ...
    public function store(BrandRequest $request): JsonResponse
    {
        ...
        // check for duplicate
        $exist = Brand::where(function (Builder $query) use ($validated) {
    ...
    public function show(string $id): JsonResponse
    {
        ...
        // get data
        $data = Brand::where('id', $validated['id'])->select(['code', 'name', 'is_active'])->first();
    ...
    public function destroy(Request $request)
    {
        ...
        $validated = Validator::make($ids, [
            'ids' => ['required', "array"],
            'ids.*' => ['required', "uuid", Rule::exists('brands', 'id')],
        ])->validated();
    ...
    public function import(Request $request, OpenSpoutHelper $openSpout): JsonResponse
    {
        ...
        try {
            ...
            foreach ($rows as $key => $row) {
                ...
                // input validator
                $validator = Validator::make($row, (new BrandRequest())->rules());
                ...
                // get data
                $isExist = Brand::whereRaw("LOWER(code)=?", [str($validated['code'])->lower()])->exists();
    ...
    public function export(Request $request, OpenSpoutHelper $openSpout): JsonResponse
    {
        ...
        // create query
        $query = Brand::select(['code', 'name', 'is_active'])
        ...
        // variables
        $fileName = now()->format('YmdHis') . "_basics_brands.xlsx";
    ...
    public function lov(Request $request): View
    {
        ...
        $data['srcURL'] = route('dt.basics.brands');

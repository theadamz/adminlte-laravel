...
class UserController extends Controller
{
    ...
    public function datatable(Request $request): JsonResponse
    {
        $queries = DB::table("users", "u")->leftJoin("roles AS r", "r.id", "=", "u.role_id")
            ->selectRaw("u.id, u.email, u.name, r.name AS role_name, u.timezone, u.is_active")
    ...
    public function store(UserCreateRequest $request): JsonResponse
    {
        ...
        $exist = User::where(function (Builder $query) use ($validated) {
            $query->whereRaw('LOWER(email)=?', [str($validated['email'])->lower()]);
        })->exists();
    ...
    public function storeRegister(SignUpRequest $request): RedirectResponse
    {
        ...
        // get role
        $role = Role::whereRaw('LOWER(name)=?', ['user'])->first();
    ...
    public function show(string $id): JsonResponse
    {
        ...
        // get data
        $data = User::where('id', $validated['id'])->select(['email', 'name', 'role_id', 'timezone', 'is_active'])->first();
    ...
    public function select2(Request $request): JsonResponse
    {
        ...
        // query
        $queries = User::select('id', 'name', 'email')
            ->where(function ($query) use ($request) {
                $query->orWhere('name', 'ilike', "%{$request->keyword}%")->orWhere('email', 'ilike', "%{$request->keyword}%");

...
function initDataTable() {
    ...
    // DataTable refresh
    document.querySelector('#refresh').addEventListener('click', () => retriveUserAccessses());
}
...
async function saveData() {
    if ($('#access_lists').val().length <= 0 || !$('#user').val()) {
        MsgBox.Notification('Select user and access.');
        return;
    }
    ...
    // send request
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses`, 'POST', _data2Send);
...
async function editData(id = null) {
    ...
    // send request
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses/${$('#user').val()}/${_id}`, 'GET');
...
async function savePermissions() {
    ...
    // send request
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses/${$('#user').val()}`, "PUT", _data2Send);
...
async function duplicateData() {
    ...
    // send request
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses/duplicate`, 'POST', _data2Send);
...
async function deleteData() {
    ...
    // send request
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses`, "DELETE", _data2Send);
...
function formDuplicateClear() {
    _data2Send = null;
    $('#from_user').val(null).empty().trigger('change');
    $('#to_user').val(null).empty().trigger('change');
...
async function retriveUserAccessses() {
    ...
    // ambil data
    const response = await axiosCustom(`${_baseURL}/configs/user-accesses/${$('#user').val()}`, 'GET', null);

...
<head>
    <base href="{{ url('/') }}" />
    <title>{{ config('setting.general.web_name_short') }} {{ !empty($title) ? '- ' . $title : '' }}</title>
    <meta charset="utf-8" />
    ...
    @once
        ...
        <link rel="stylesheet"
            href="{{ url('assets/vendor/plugins/datatables-responsive/css/responsive.bootstrap4.min.css') }}">
        ...
        <link rel="stylesheet"
            href="{{ url('assets/vendor/plugins/tempusdominus-bootstrap-4/css/tempusdominus-bootstrap-4.min.css') }}">
        <!-- Perfect scrollbar -->
        ...
        @foreach (GeneralHelper::getadditionalVendorCSS() as $fileLocation)
            <link href="{{ $fileLocation }}" rel="stylesheet" type="text/css" />
        @endforeach
        @foreach (GeneralHelper::getadditionalCSS() as $fileLocation)
            @vite($fileLocation)
        @endforeach
...
<body class="layout-top-nav layout-fixed layout-navbar-fixed">
    ...
    <div class="wrapper">
        @once
            @include('templates.layout.client._preloader')
            @include('templates.layout.client._navbar')
        @endonce
        @include('templates.layout.client._default')
        @once
            @include('templates.layout.client._footer')
        @endonce
    </div>
    ...
    @once
        ...
        <!-- Tempusdominus Bootstrap 4 -->
        <script src="{{ url('assets/vendor/plugins/tempusdominus-bootstrap-4/js/tempusdominus-bootstrap-4.min.js') }}">
        </script>
        <!-- Perfect scrollbar -->
        ...
        <!--begin::Custom Javascript(used for this page only)-->
        <script src="{{ url('assets/js/lib/idle-timer/idle-timer.min.js') }}"></script>
        @foreach (GeneralHelper::getAdditionalVendorJS() as $fileLocation)
            <script src="{{ $fileLocation }}"></script>
        @endforeach
        @foreach (GeneralHelper::getAdditionalJS() as $fileLocation)
            @vite($fileLocation)
        @endforeach
